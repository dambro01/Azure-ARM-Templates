# https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.compute/encrypt-running-linux-vm
...
####################################################################################################################################
    ...
    if($aadAppName)
    {
	    ...
	    $ADApp = New-AzADApplication -DisplayName $aadAppName -HomePage $defaultHomePage -StartDate $now -EndDate $oneYearFromNow;
            ...
            $servicePrincipal = New-AzADServicePrincipal -AppId $ADApp.AppId;
            $SvcPrincipals = (Get-AzADServicePrincipal -SearchString $aadAppName);
            if(-not $SvcPrincipals)
            {
