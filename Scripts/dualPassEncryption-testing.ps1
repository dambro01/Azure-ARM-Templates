Param( [Parameter(Mandatory = $true, HelpMessage = 'Name of the resource group to which the KeyVault belongs to. A new resource group with this name will be created if one doesn''t exist')] [ValidateNotNullOrEmpty()] [string]$resourceGroupName, [Parameter(Mandatory = $true, HelpMessage = 'Name of the KeyVault in which encryption keys are to be placed. A new vault with this name will be created if one doesn''t exist ')] [ValidateNotNullOrEmpty()] [string]$keyVaultName, [Parameter(Mandatory = $true, HelpMessage = 'Location of the KeyVault. Important note: Make sure the KeyVault and VMs to be encrypted are in the same region / location. ')] [ValidateNotNullOrEmpty()] [string]$location, [Parameter(Mandatory = $false, HelpMessage = 'Identifier of the Azure subscription to be used ')] [ValidateNotNullOrEmpty()] [string]$subscriptionId, [Parameter(Mandatory = $false, HelpMessage = 'Name of the AAD application that will be used to write secrets to KeyVault. A new application with this name will be created if one doesn''t exist. If this app already exists, pass aadClientSecret parameter to the script ')] [ValidateNotNullOrEmpty()] [string]$aadAppName, [Parameter(Mandatory = $false, HelpMessage = 'Client secret of the AAD application that was created earlier ')] [ValidateNotNullOrEmpty()] [string]$aadClientSecret, [Parameter(Mandatory = $false, HelpMessage = 'Name of optional key encryption key in KeyVault. A new key with this name will be created if one doesn''t exist ')] [ValidateNotNullOrEmpty()] [string]$keyEncryptionKeyName); $VerbosePreference = 'Continue'; $ErrorActionPreference = 'Stop'; $SvcPrincipals = (Get-AzADServicePrincipal -SearchString $aadAppName); if (-not $SvcPrincipals) {
 $identifierUri = [string]::Format("http://localhost:8080/{0}", [Guid]::NewGuid().ToString('N')); $defaultHomePage = 'http://contoso.com'; $now = [System.DateTime]::Now; $oneYearFromNow = $now.AddYears(1); $aadClientSecret = [Guid]::NewGuid().ToString(); Write-Host 'Creating new AAD application ($aadAppName) '; $ADApp = New-AzADApplication -DisplayName $aadAppName -HomePage $defaultHomePage -StartDate $now -EndDate $oneYearFromNow; $servicePrincipal = New-AzADServicePrincipal -AppId $ADApp.AppId; $SvcPrincipals = (Get-AzADServicePrincipal -SearchString $aadAppName); if (-not $SvcPrincipals) {
		Write-Error 'Failed to create AAD app $aadAppName. Please log in to Azure using Connect-AzAccount and try again '; return;
	} $aadClientID = $servicePrincipal.AppId; Write-Host 'Created a new AAD Application ($aadAppName) with ID: $aadClientID ';
}
else {
 if (-not $aadClientSecret) {
		Write-Error 'Aad application ($aadAppName) was already created. Re-run the script by supplying aadClientSecret parameter with corresponding secret from https://manage.windowsazure.com portal '; return;
	} $aadClientID = $SvcPrincipals[0].AppId;
} Try {
 $resGroup = Get-AzResourceGroup -Name $resourceGroupName -ErrorAction SilentlyContinue;
}
Catch [System.ArgumentException] {
 Write-Host 'Couldn''t find resource group: ($resourceGroupName) '; $resGroup = $null;
} if (-not $resGroup) {
 Write-Host 'Creating new resource group: ($resourceGroupName) '; $resGroup = New-AzResourceGroup -Name $resourceGroupName -Location $location; Write-Host 'Created a new resource group named $resourceGroupName to place keyVault ';
} Try {
 $keyVault = Get-AzKeyVault -VaultName $keyVaultName -ErrorAction SilentlyContinue;
}
Catch [System.ArgumentException] {
 Write-Host 'Couldn''t find Key Vault: $keyVaultName '; $keyVault = $null;
} if (-not $keyVault) {
 Write-Host 'Creating new key vault: ($keyVaultName) '; $keyVault = New-AzKeyVault -VaultName $keyVaultName -ResourceGroupName $resourceGroupName -Sku Standard -Location $location; Write-Host 'Created a new KeyVault named $keyVaultName to store encryption keys ';
} if ($aadAppName) {
 ...
 Catch [Microsoft.Azure.KeyVault.KeyVaultClientException] {
		Write-Host 'Couldn''t find key encryption key named : $keyEncryptionKeyName in Key Vault: $keyVaultName '; $kek = $null;
	} if (-not $kek) {
		Write-Host 'Creating new key encryption key named:$keyEncryptionKeyName in Key Vault: $keyVaultName '; $kek = Add-AzKeyVaultKey -VaultName $keyVaultName -Name $keyEncryptionKeyName -Destination Software -ErrorAction SilentlyContinue; Write-Host 'Created key encryption key named:$keyEncryptionKeyName in Key Vault: $keyVaultName ';
	} $keyEncryptionKeyUrl = $kek.Key.Kid;
} Write-Host 'Please note down below details that will be needed to enable encryption on your VMs ' -ForegroundColor Green; if ($aadAppName) {
 Write-Host ' aadClientID: $aadClientID ' -ForegroundColor Green; Write-Host ' aadClientSecret: $aadClientSecret ' -ForegroundColor Green;
} Write-Host ' DiskEncryptionKeyVaultUrl: $diskEncryptionKeyVaultUrl ' -ForegroundColor Green; Write-Host ' DiskEncryptionKeyVaultId: $keyVaultResourceId ' -ForegroundColor Green; if ($keyEncryptionKeyName) {
 Write-Host ' KeyEncryptionKeyURL: $keyEncryptionKeyUrl ' -ForegroundColor Green; Write-Host ' KeyEncryptionKeyVaultId: $keyVaultResourceId' -ForegroundColor Green;
} $allVMs = Get-AzVM -ResourceGroupName $resourceGroupName; foreach ($vm in $allVMs) {
 if ($vm.Location.ToLower() -ne $keyVault.Location.ToLower()) {
		Write-Error 'To enable AzureDiskEncryption, VM and KeyVault must belong to same subscription and same region. vm Location: $($vm.Location.ToLower()), keyVault Location: $($keyVault.Location.ToLower())'; return;
	} Write-Host 'Encrypting VM: $($vm.Name) in ResourceGroup: $($vm.ResourceGroupName)' -ForegroundColor Green; if ($aadAppName) {
		if (-not $kek) {
